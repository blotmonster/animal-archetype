...
const animals = {
  "Axolotl": {
    emoji:"🦎",
    blurb:"Calm, adaptable, and quietly adorable. You don’t need to be loud to be powerful.",
...
while (questions.length < 15) {
  questions.push(...questions.slice(0, Math.min(5,15-questions.length)));
}
...
const imageRounds = [
  {
    ...
    options:[
      { media:imageHTML("./images/a-glasshouse.jpg","glasshouse"), t:{calm:2,order:1}},
      { media:imageHTML("./images/b-neon.jpg","neon"), t:{bold:1,calm:-1,order:-1}}
    ]
  },
  {
    prompt:"Which feels more like your mind?",
    options:[
      { media:imageHTML("./images/c-library.jpg","library"), t:{curious:2,order:1}},
      { media:imageHTML("./images/d-ocean.jpg","ocean"), t:{calm:1,bold:1}}
    ]
  },
  {
    prompt:"Your friend vibe?",
    options:[
      { media:imageHTML("./images/e-campfire.jpg","campfire"), t:{social:2,warm:2}},
      { media:imageHTML("./images/f-summit.jpg","summit"), t:{social:-2,bold:1}}
    ]
  },
  {
    prompt:"What feels satisfying?",
    options:[
      { media:imageHTML("./images/g-clockwork.jpg","clockwork"), t:{order:2}},
      { media:imageHTML("./images/h-garden.jpg","garden"), t:{order:-2,curious:2}}
    ]
  },
  {
    prompt:"Your protective instinct?",
    options:[
      { media:imageHTML("./images/i-shield.jpg","shield"), t:{bold:1,order:2}},
      { media:imageHTML("./images/j-soft.jpg","soft"), t:{warm:2,calm:2}}
    ]
  }
];
...
function renderQuestion(){
  ...
  app.innerHTML=`
  <h2>${q.text}</h2>
  ${q.answers.map((a,i)=>`<button onclick="pickQ(${i})">${a.label}</button>`).join("")}
  `;
}
function pickQ(i){
  qPicks[qIndex]=i;
...
function renderImage(){
  ...
  app.innerHTML=`
  <h2>${r.prompt}</h2>
  ${r.options.map((o,i)=>`<div onclick="pickI(${i})">${o.media}</div>`).join("")}
  `;
}
function pickI(i){
  iPicks[iIndex]=i;
  if(iIndex<imageRounds.length-1){iIndex++;renderImage();}
  else renderResult();
}
function renderResult(){
  const traits=emptyTraitVector();
  questions.forEach((q,qi)=>addTraits(traits,q.answers[qPicks[qi]].t));
  imageRounds.forEach((r,ri)=>addTraits(traits,r.options[iPicks[ri]].t));

  let best=null,bestScore=-Infinity;
  Object.entries(animals).forEach(([name,a])=>{
    let score=0;
    TRAITS.forEach(t=>score-=Math.abs((traits[t]||0)-(a.profile[t]||0)));
