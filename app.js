...
// ---------------- IMAGE HELPER ----------------
// IMPORTANT: Images are in repo root (same folder as index.html)
function imageHTML(fileName, alt){
  return `<img src="./${fileName}" alt="${alt}" loading="lazy" />`;
}
...
const imageRounds = [
  {
    ...
    options:[
      { media:imageHTML("a-glasshouse.jpg","glasshouse"), t:{calm:2,order:1}},
      { media:imageHTML("b-neon.jpg","neon"), t:{bold:1,calm:-1,order:-1}}
    ]
  },
  {
    prompt:"Which feels more like your mind?",
    options:[
      { media:imageHTML("c-library.jpg","library"), t:{curious:2,order:1}},
      { media:imageHTML("d-ocean.jpg","ocean"), t:{calm:1,bold:1}}
    ]
  },
  {
    prompt:"Your friend vibe?",
    options:[
      { media:imageHTML("e-campfire.jpg","campfire"), t:{social:2,warm:2}},
      { media:imageHTML("f-summit.jpg","summit"), t:{social:-2,bold:1}}
    ]
  },
  {
    prompt:"What feels satisfying?",
    options:[
      { media:imageHTML("g-clockwork.jpg","clockwork"), t:{order:2}},
      { media:imageHTML("h-garden.jpg","garden"), t:{order:-2,curious:2}}
    ]
  },
  {
    prompt:"Your protective instinct?",
    options:[
      { media:imageHTML("i-shield.jpg","shield"), t:{bold:1,order:2}},
      { media:imageHTML("j-soft.jpg","soft"), t:{warm:2,calm:2}}
    ]
  }
];
...
function renderImage(){
  ...
  app.innerHTML = `
    <h2>${r.prompt}</h2>
    ${r.options.map((o,i)=>`
      <div onclick="pickI(${i})" style="cursor:pointer;">
        ${o.media}
      </div>
    `).join("")}
  `;
...
function renderResult(){
  ...
  questions.forEach((q,qi)=>{
    addTraits(traits, q.answers[qPicks[qi]].t);
  });

  imageRounds.forEach((r,ri)=>{
    addTraits(traits, r.options[iPicks[ri]].t);
  });
